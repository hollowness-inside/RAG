...
impl OllamaEmbedder {
    pub fn new<A: IntoUrl, S: ToString>(addr: A, model: S) -> RagResult<Self> {
...
impl Embedder for OllamaEmbedder {
    fn embed(&self, prompt: &str) -> RagResult<Vec<f32>> {
        let url = self
            .addr
            .join("/api/embeddings")
        ...
        serde_json::from_reader::<_, HashMap<String, Vec<f32>>>(response)
